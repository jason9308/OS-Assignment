...
void *thread1(void *arg){
    ...
    //printf("Thread Message: %s, TID: %ld\n", "Thread 1 says hello!", syscall(SYS_gettid));
    ...
    // Write the message stored in `data` to the proc file
    fprintf(proc_file, "%s\n", data);
    fclose(proc_file);
    /****************/ 
...
void *thread2(void *arg){
    ...
    /*YOUR CODE HERE*/
    /* Hint: Write data into proc file.*/
    // Open /proc/Mythread_info for writing
    //pthread_spin_lock(&lock);
    FILE *proc_file = fopen("/proc/Mythread_info", "w");
    if (proc_file == NULL) {
        perror("Failed to open /proc/Mythread_info");
