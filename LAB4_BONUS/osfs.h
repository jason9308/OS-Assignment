...
struct osfs_data_block {
    char data[BLOCK_SIZE - sizeof(uint32_t)]; // Actual data stored in the block
    uint32_t next_block;                      // Index of the next data block, -1 if none
};
...
struct osfs_inode {
    ...
    uint32_t first_block;                   // Simplified handling, single data block pointer
};
...
/* 
 * Frees all data blocks associated with the given inode 
 * by traversing the linked allocation chain and releasing each block
 */
void osfs_free_data_block_chain(struct inode *inode);  
void osfs_destroy_inode(struct inode *inode);
int osfs_append_data_block(struct osfs_sb_info *sb_info, uint32_t *last_block, uint32_t *new_block);
int osfs_traverse_data_blocks(struct osfs_sb_info *sb_info, uint32_t start_block, void (*callback)(void *));
